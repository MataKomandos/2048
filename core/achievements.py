...
class Achievements:
    ...
    # Lista wszystkich dostępnych osiągnięć
    ACHIEVEMENTS = {
        "first_win": {
    ...
    def unlock_achievement(self, player_nickname: str, achievement_id: str) -> bool:
        """
        Odblokuj osiągnięcie dla gracza.
        
        Args:
            player_nickname (str): Nick gracza
            achievement_id (str): ID osiągnięcia
        
        Returns:
            bool: True jeśli osiągnięcie zostało odblokowane
        """
    ...
    def get_player_points(self, player_nickname: str) -> int:
        """
        Pobierz łączną liczbę punktów gracza.
        
        Args:
            player_nickname (str): Nick gracza
        
        Returns:
            int: Łączna liczba punktów
        """
        player_achievements = self.player_achievements.get(player_nickname, [])
    ...
    def check_game_achievements(self, player_nickname: str, game_data: Dict) -> List[str]:
        """
        Sprawdź i odblokuj osiągnięcia na podstawie danych z gry.
        
        Args:
            player_nickname (str): Nick gracza
            game_data (Dict): Dane z gry do sprawdzenia
        
        Returns:
            List[str]: Lista nowo odblokowanych osiągnięć
        """
        unlocked = []
        
        # Sprawdź różne warunki osiągnięć
        ...
        # Sprawdź osiągnięcia specyficzne dla trybów
    ...
    def display_unlocked_achievements(self, unlocked: List[str]) -> None:
        """
        Wyświetl nowo odblokowane osiągnięcia.
        
        Args:
            unlocked (List[str]): Lista ID odblokowanych osiągnięć
        """
        if unlocked:
